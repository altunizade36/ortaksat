-- Komisyon bonusu (teşvik): satıcı, ilk N satışı yapan ortaklara komisyona ek
-- bir bonus taahhüt edebilir. Platform para tutmaz; kayıt amaçlıdır.
alter table public.listings
  add column if not exists bonus_amount numeric,
  add column if not exists bonus_quota integer;

-- Public view'e bonus kolonlarını ekle (yalnızca sona eklenebilir).
