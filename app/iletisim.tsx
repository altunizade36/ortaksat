...
export default function ContactScreen() {
  ...
  return (
    <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ paddingBottom: 0, paddingTop: 16 }} style={{ backgroundColor: colors.background }}>
      <Head>
        <title>İletişim — OrtakSat destek, şikayet ve başvuru</title>
        <meta name="description" content="OrtakSat ile iletişim: şikayet, dolandırıcılık bildirimi, KVKK/yasal başvuru ve genel destek için destek@ortaksat.com. Ortaksat aracı platformdur; iletişim e-posta üzerinden yürür." />
        <meta property="og:title" content="İletişim — OrtakSat" />
        <meta property="og:description" content="Şikayet, bilgi, yasal başvuru ve destek: destek@ortaksat.com" />
      </Head>
      <View style={{ alignSelf: "center", gap: 18, maxWidth: 820, paddingHorizontal: 20, width: "100%" }}>
        ...
        <View style={{ alignItems: "center", flexDirection: "row", gap: 4 }}>
          ...
          <Text style={{ color: colors.muted, fontSize: 12.5, fontWeight: "700" }}>İletişim</Text>
        </View>
        ...
        <View style={{ gap: 8 }}>
          ...
          <Text style={{ color: colors.muted, fontSize: 15.5, fontWeight: "600", lineHeight: 23 }}>
            OrtakSat bir aracı platformdur; çağrı merkezi/telefon hattı işletmez. Şikayet, bilgi, yasal başvuru ve destek talepleri için bize e-posta ile ulaşabilirsin. Genellikle 1–3 iş günü içinde dönüş yaparız.
          </Text>
        </View>
        ...
        <Pressable onPress={openMail} style={({ pressed }) => ({ backgroundColor: colors.primarySoft, borderColor: colors.primary, borderRadius: 16, borderWidth: 1, gap: 6, opacity: pressed ? 0.9 : 1, padding: 20 })}>
          <View style={{ alignItems: "center", flexDirection: "row", gap: 8 }}>
            ...
            <Text style={{ color: colors.primaryDark, fontSize: 13, fontWeight: "900", textTransform: "uppercase" }}>E-posta</Text>
          </View>
          ...
          <Text style={{ color: colors.muted, fontSize: 13, fontWeight: "600" }}>Yazmak için dokun — varsayılan e-posta uygulaman açılır.</Text>
        </Pressable>
          ...
          {REASONS.map((r) => (
            <View key={r.title} style={{ backgroundColor: colors.surface, borderColor: colors.line, borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 12, padding: 16 }}>
              ...
              <View style={{ flex: 1, gap: 3, minWidth: 0 }}>
                <Text style={{ color: colors.ink, fontSize: 15, fontWeight: "900" }}>{r.title}</Text>
                <Text style={{ color: colors.muted, fontSize: 13.5, fontWeight: "500", lineHeight: 20 }}>{r.body}</Text>
              </View>
            </View>
          ))}
        </View>
        ...
        <View style={{ backgroundColor: colors.surfaceAlt, borderRadius: 12, gap: 4, padding: 14 }}>
          <Text style={{ color: colors.ink, fontSize: 13.5, fontWeight: "800" }}>Önemli</Text>
          <Text style={{ color: colors.muted, fontSize: 13, fontWeight: "500", lineHeight: 19 }}>
            OrtakSat ödeme tutmaz, kargo/teslimat yürütmez. Satış, ödeme ve teslimat satıcı ile alıcı arasında gerçekleşir. Alışverişle ilgili anlaşmazlıklarda önce karşı tarafla platform mesajlaşmasında iletişim kurman, kayıtların korunması açısından önemlidir.
          </Text>
          <Link href="/legal" asChild>
            <Pressable style={{ marginTop: 4 }}><Text style={{ color: colors.primaryDark, fontSize: 13, fontWeight: "900" }}>Yasal & Destek belgeleri →</Text></Pressable>
          </Link>
        </View>
      </View>
      <WebFooter />
