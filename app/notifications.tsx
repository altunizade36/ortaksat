...
const typeMeta: Record<NotificationType, { label: string; tint: string; color: string }> = {
  ...
  offer: { label: "Teklif", tint: colors.goldSoft, color: colors.goldInk },
  question: { label: "Soru", tint: colors.infoSoft, color: colors.info },
...
function NotificationsScreenInner() {
  ...
  if (isWideWeb) {
    ...
    const counts: Record<NotificationType, number> = { application: 0, lead: 0, sale: 0, message: 0, payout: 0, review: 0, price_drop: 0, sold: 0, follow: 0, offer: 0, question: 0, system: 0 };
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} contentContainerStyle={{ backgroundColor: colors.background, flexGrow: 1, paddingBottom: 0 }} style={{ backgroundColor: colors.background }}>
        ...
        <View style={{ alignItems: "flex-end", flexDirection: "row", gap: 12 }}>
          ...
          <Link href="/profile-edit" asChild>
            <Pressable accessibilityRole="button" accessibilityLabel={translateCopy("Ayarlar", language)} style={{ alignItems: "center", backgroundColor: colors.surface, borderColor: colors.line, borderRadius: 10, borderWidth: 1, height: 38, justifyContent: "center", width: 40 }}>
