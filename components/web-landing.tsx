  ...
  return (
    <View dataSet={{ reveal: "1" }} style={{ gap: 16, marginTop: 8 }}>
      <View style={{ alignItems: "flex-end", flexDirection: "row", gap: 10, justifyContent: "space-between" }}>
        <View style={{ gap: 2 }}>
          <Text style={{ color: colors.ink, fontSize: 24, fontWeight: "900" }}>{translateCopy("Kategoriler", language)}</Text>
          <Text style={{ color: colors.muted, fontSize: 15, fontWeight: "600" }}>{translateCopy("İlgilendiğin alanı seç, en iyi fırsatları kaçırma.", language)}</Text>
        </View>
        <Link href="/explore" asChild>
          <Pressable style={{ alignItems: "center", flexDirection: "row", gap: 4 }}>
            <Text style={{ color: colors.primaryDark, fontSize: 14, fontWeight: "900" }}>{translateCopy("Tüm kategorileri gör", language)}</Text>
            ...
            <Link key={category.key} href={{ pathname: "/explore", params: { q: category.label } }} asChild>
              ...
              >
                ...
                <Text numberOfLines={1} style={{ color: colors.muted, fontSize: 12, fontWeight: "700" }}>
                  {count > 0 ? `${count} ${translateCopy("ilan", language)}` : `${category.subcategories.length} ${translateCopy("alt kategori", language)}`}
                </Text>
              </Pressable>
            </Link>
          );
...
export function WebHowItWorks() {
  ...
  return (
    <View dataSet={{ reveal: "1" }} style={{ gap: 16, marginTop: 8 }}>
      <View style={{ gap: 4 }}>
        <Text style={{ color: colors.ink, fontSize: 24, fontWeight: "900" }}>{translateCopy("Nasıl çalışır?", language)}</Text>
        <Text style={{ color: colors.muted, fontSize: 15, fontWeight: "600" }}>
          {translateCopy("Ortak satış üç adımda: aç, paylaştır, kazan.", language)}
        </Text>
      </View>
      <View style={{ flexDirection: "row", gap: 16 }}>
        {steps.map((step) => (
          ...
          >
            ...
            <Text style={{ color: colors.ink, fontSize: 18, fontWeight: "900" }}>{translateCopy(step.title, language)}</Text>
            <Text style={{ color: colors.muted, fontSize: 15, fontWeight: "600", lineHeight: 22 }}>{translateCopy(step.body, language)}</Text>
          </View>
        ))}
      </View>
    </View>
...
export function WebWhy() {
  ...
  return (
    <View dataSet={{ reveal: "1" }} style={{ gap: 16, marginTop: 8 }}>
      <View style={{ gap: 2 }}>
        <Text style={{ color: colors.ink, fontSize: 24, fontWeight: "900" }}>{translateCopy("Neden ortaksat?", language)}</Text>
        <Text style={{ color: colors.muted, fontSize: 15, fontWeight: "600" }}>{translateCopy("İki taraf da kazanır: satıcı büyür, ortak komisyon alır.", language)}</Text>
      </View>
        ...
        {sides.map((side) => (
          ...
          >
            <View style={{ alignItems: "center", flexDirection: "row", gap: 12 }}>
              ...
              <Text style={{ color: colors.ink, fontSize: 20, fontWeight: "900" }}>{translateCopy(side.title, language)}</Text>
            </View>
            <View style={{ gap: 10 }}>
              {side.points.map((point) => (
                <View key={point} style={{ alignItems: "flex-start", flexDirection: "row", gap: 10 }}>
                  <MaterialCommunityIcons name="check-circle" size={18} color={colors.primary} />
                  <Text style={{ color: colors.muted, flex: 1, fontSize: 14, fontWeight: "600", lineHeight: 21 }}>{translateCopy(point, language)}</Text>
                </View>
              ))}
            </View>
            <Link href={side.href} asChild>
              <Pressable style={{ alignItems: "center", alignSelf: "flex-start", backgroundColor: colors.primary, borderRadius: 12, flexDirection: "row", gap: 8, paddingHorizontal: 20, paddingVertical: 12 }}>
                <Text style={{ color: "#FFFFFF", fontSize: 14, fontWeight: "900" }}>{translateCopy(side.cta, language)}</Text>
